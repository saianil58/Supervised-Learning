# -*- coding: utf-8 -*-
"""
Created on Mon Oct 26 11:21:55 2020
...
def run(fold):
    ...
    features = [ f for f in df.columns if f not in ("id",config.TARGET_COL_NAME,"kfold")]
    ...
    model.fit(x_train,df_train[config.TARGET_COL_NAME].values)
    ...
    # get auc score
    auc = metrics.roc_auc_score(df_valid[config.TARGET_COL_NAME].values, valid_preds)
